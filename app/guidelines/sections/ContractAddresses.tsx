...
import {
  GUIDELINES_DEPLOYED_CONTRACTS,
  GUIDELINES_EXTERNAL_CONTRACTS,
  arbiscanAddressUrl,
...
function ContractTable({
  ...
  rows: readonly { name: string; address: string }[];
}) {
  return (
    <GuidelinesScrollTable caption={caption}>
      <thead>
        <tr className="border-b border-slate-200 bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500">
          <th className="px-4 py-3">Contract</th>
          <th className="px-4 py-3">{addressColumnLabel}</th>
...
export default function ContractAddresses() {
  return (
    <div className="space-y-8">
      <ContractTable
        caption="Deployed Contracts on Arbitrum"
        addressColumnLabel="Address (Arbitrum)"
        rows={GUIDELINES_DEPLOYED_CONTRACTS}
      />
      <ContractTable
        caption="External Dependencies on Arbitrum Sepolia"
        addressColumnLabel="Address (Arbitrum Sepolia)"
        rows={GUIDELINES_EXTERNAL_CONTRACTS}
      />
    </div>
  );
}
